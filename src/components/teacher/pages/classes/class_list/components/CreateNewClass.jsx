...
const CreateNewClass = React.forwardRef((props, ref) => {
    ...
    function doSchedulesConflict(schedule, newSchedule) {
        ...
        const isTimeConflict =
            (start1.isBefore(end2) && end1.isAfter(start2)) || (start2.isBefore(end1) && end2.isAfter(start1));
                ...
                try {
                    ...
                    if (schedules !== null) {
                        ...
                        newClass.schedules.forEach((newSched) => {
                            const hasConflict = schedules.some((schedule) => doSchedulesConflict(schedule, newSched));
        ...
        onSubmit: async (values, { resetForm }) => {
            ...
            if (values.schedules.length === 0) {
                ...
                checkClassDuplicated(values.section)
                    .then((hasConflicts) => {
                        if (hasConflicts) {
                            enqueueSnackbar("Invalid class to create. This class already exists.", {
                                variant: "error",
                            });
                        } else {
    ...
    }, [schedState, addNewClassFormik.values]);
                        ...
                        >
                            ...
                            >
                                <TextField
                                    ...
                                    error={addNewClassFormik.touched.title && Boolean(addNewClassFormik.errors.title)}
                                    helperText={addNewClassFormik.touched.title && addNewClassFormik.errors.title}
                                />
                                <FormControl
                                    size="small"
                                    sx={{
                            ...
                            <TextField
                                ...
                                helperText={
                                    addNewClassFormik.touched.description && addNewClassFormik.errors.description
                                }
                            />
                            <TextField
                                ...
                                onChange={addNewClassFormik.handleChange}
                                onBlur={addNewClassFormik.handleBlur}
                                error={addNewClassFormik.touched.section && Boolean(addNewClassFormik.errors.section)}
                                helperText={addNewClassFormik.touched.section && addNewClassFormik.errors.section}
                            />
                            <Collapse
                                in={Boolean(addNewClassFormik.touched.schedules && addNewClassFormik.errors.schedules)}
                                unmountOnExit
                            >
                                <Box
                                    sx={{
                            ...
                            <Box sx={{ display: "flex", flexDirection: "row-reverse", mt: 3 }}>
                                <LoadingButton loading={submitLoading} type="submit" sx={{ ml: 1 }} variant="contained">
                                    Create
                                </LoadingButton>
                                <Button
                                    onClick={() => {
